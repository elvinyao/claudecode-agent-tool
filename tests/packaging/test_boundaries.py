    ...
    for source in CORE_SOURCE.rglob("*.py"):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                continue
            if any(
                name == "trivy_ai_report" or name.startswith("trivy_ai_report.") for name in names
            ):
...
def test_trivy_is_declared_as_domain_entry_point() -> None:
    manifest = (ROOT / "plugins" / "trivy-ai-report" / "pyproject.toml").read_text(encoding="utf-8")
...
def test_base_core_metadata_keeps_optional_transports_and_sdks_out() -> None:
    ...
    assert project["optional-dependencies"]["antigravity"] == ["google-antigravity>=0.1.13,<0.2"]
    assert project["optional-dependencies"]["codex"] == ["openai-codex>=0.144.4,<1"]
    ...
    top_level_imports = {
        node.module or "" for node in cli_tree.body if isinstance(node, ast.ImportFrom)
    }
